...
| `SPI` | Schedule Performance Index = EV/PV = BCWP/BCWS (PV time-phased by linear baseline spend); ≥ 0.95 traffic-light management watch level (NOT a DCMA-14 number). Needs schema v1.1.0 `budgeted_cost`+`baseline_start`; SKIPPED without them | EVM index per ANSI/EIA-748 & NASA-EVM (SPI = BCWP/BCWS) | method cited (practice); 0.95 is a management traffic-light convention, not DCMA; GAO Cost Est. & Assessment Guide / NDIA IPMD favour trend → 0.95 is a program-configurable default; source-pending |
| `SPI(t)` | Earned-Schedule SPI(t) = ES/AT; ES inverts the baseline PV curve to the time matching current EV; ≥ 0.95 traffic-light management watch level. Needs schema v1.1.0 fields; SKIPPED without them | EVM index per ANSI/EIA-748 & NASA-EVM / Lipke "Earned Schedule" | method cited (practice); 0.95 is a management traffic-light convention, not DCMA; GAO / NDIA IPMD favour trend → 0.95 is a program-configurable default; source-pending |
| `CEI` Current Execution Index | Per-period count ratio: tasks that actually finished in `(prior.status_date, current.status_date]` ÷ non-summary, incomplete tasks the prior version FORECAST (frozen `Task.finish`) to finish in that period. Finish-only; per-period; capped at 1.0; needs ≥2 status-dated versions (else "insufficient data"). Threshold ≥0.95 | PASEG 10.4.5; NDIA IPMD "Managing Programs Using Predictive Measures" | method cited; **0.95 threshold source-pending (VERIFY** — NDIA prefers a >75th-pct trend; CEI is already reported per-period, i.e. as a trend, with 0.95 as the per-period gate); auto-snapshot capture is a **tool-original method** (metric standard, capture ours) |
